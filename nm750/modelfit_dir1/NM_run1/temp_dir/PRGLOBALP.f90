...
!
        INTEGER(KIND=ISIZE) :: NP,NBP,YFORM,MAXKF,IFORM(PG1),IDC,IDO,ISV(PC),LOGUNT,KDES,   & ! IDC -> IDD; IDD only used in ADVAN10
                               ...
                               MCNTR,KAES,NOPROB,PKPROB,ERPROB,ANRD,NY,SVI(PC)
!     
...
!
        INTEGER(KIND=ISIZE) :: PNEWIF,NACTIV,ISFINL,A_0FLG,IDXETA(0:PE),MTNOW,       &  ! M -> IDXETA
                               MTPAST(0:PCT),MTNEXT(0:PCT),NEVENT,A_UFLG            ! N -> NEVENT
!
...
!
      ...
      INTEGER(KIND=ISIZE) :: IPAR(PC+1),IPAST(MAXNRDS),NRDS,NLAG_TIMES,NMTTIME

      REAL(KIND=DPSIZE)   :: LAG_TIMES(PC+1),MTTIME((4*MAXNRDS+2)*PAST_SIZE)
      DATA NLAG_TIMES /0/
      DATA NMTTIME /0/
...
!      REAL(KIND=DPSIZE)   :: PAST( (4*MAXNRDS+2)*PAST_SIZE)
      REAL(KIND=DPSIZE), TARGET   :: RPAR(PC+1)

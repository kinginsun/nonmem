...
!     
      ...
      KERR=0; JDEF=PD+1; TREC(PD+1)=0.0; T2=-1.0D+300
!          
...
!    
!      IF (T2 < 0.0) THEN
! The following 3 statements are commented out in NONMEM 7.4 alpha.
! The purpose is to allow negative times in the NM-TRAN data set.
! This is change no. 7.4.0-1 15 Dec. 2013
!!!     IF (IERR+KERR == 0) WRITE (LOGUNT,8000,ERR=502)
...
! Only permit CALL=10,11,12,13 with ADVAN9
      IF (ADVID /= 9 .AND. ADVID/=15 .AND. ADVID/=17) GO TO 140
...
!        
        IF (CKSV(IC) /= 1) THEN 
          IF (IERR+KERR == 0) WRITE (LOGUNT,8000,ERR=502)
          WRITE (LOGUNT,8037,ERR=502) KREC,IC
          IERR=IERR+1
        END IF
!       
...
! the original code was perhaps more understandable:
! IF (ISV(IC).EQ.0.AND.IINST(IC).EQ.0) GO TO 250
! if the compartment starts OFF and may not receive a dose, its ok to turn off with obs
! AJB 12/2014
...
!     
 ...
 8003 FORMAT ('0DATA REC ',I11,': VALUE OF CONTINUATION DATA ITEM IS INAPPROPRIATE')
 8004 FORMAT ('0DATA REC ',I11,': MDV DATA ITEM MUST BE 1 WHEN CONTINUATION DATA ITEM IS 1')
 8005 FORMAT ('0DATA REC ',I11,': TIME DATA ITEM IS LESS THAN PREVIOUS TIME DATA ITEM')
 8006 FORMAT ('0DATA REC ',I11,': DOSE AMOUNT DATA ITEM HAS INAPPROPRIATE VALUE')
 8007 FORMAT ('0DATA REC ',I11,': RATE DATA ITEM HAS INAPPROPRIATE VALUE')
 8008 FORMAT ('0DATA REC ',I11,': STEADY STATE DATA ITEM HAS INAPPROPRIATE VALUE')
 8009 FORMAT ('0DATA REC ',I11,': INTERDOSE INTERVAL DATA ITEM HAS INAPPROPRIATE VALUE')
 8010 FORMAT ('0DATA REC ',I11,': COMPARTMENT NUMBER IS INAPPROPRIATE FOR THIS MODEL')
 8011 FORMAT ('0DATA REC ',I11,': PREDICTION COMPARTMENT NUMBER IS INAPPROPRIATE FOR THIS MODEL')
 8012 FORMAT ('0DATA REC ',I11,': CALL DATA ITEM HAS INAPPROPRIATE VALUE')
 8013 FORMAT ('0DATA REC ',I11,': EVENT IDENTIFICATION DATA ITEM HAS INAPPROPRIATE VALUE')
 8014 FORMAT ('0DATA REC ',I11,': SPECIFIED COMPARTMENT MAY NOT BE TURNED OFF WITH'/' AN OBSERVATION RECORD')
 8015 FORMAT ('0DATA REC ',I11,': OBSERVATION EVENT RECORD MAY NOT SPECIFY DOSING INFORMATION')
 8017 FORMAT ('0DATA REC ',I11,': MDV DATA ITEM MUST BE 1 FOR LAST RECORD OF DOSE EVENT')
 8018 FORMAT ('0DATA REC ',I11,': INTERDOSE INTERVAL DATA ITEM IS NON-ZERO,'/                 &
                              ' STEADY STATE AND ADDITIONAL DOSES DATA ITEMS ARE ZERO')
 8019 FORMAT ('0DATA REC ',I11,': CAN NOT TURN OFF A COMPARTMENT RECEIVING A DOSE')
 8021 FORMAT ('0DATA REC ',I11,': DOSE NOT ALLOWED IN THIS COMPARTMENT ')
 8024 FORMAT ('0DATA REC ',I11,': PK PARAMETER FOR RATE IS NOT SUPPLIED BY PK FOR'/' COMPT. NO. ',I3)
 8025 FORMAT ('0DATA REC ',I11,': DOSE AMOUNT IS ZERO')
 8026 FORMAT ('0DATA REC ',I11,': DOSE RATE, DOSE AMOUNT, INTERDOSE INTERVAL DATA ITEMS'/     &
                              ' MUTUALLY INCOMPATIBLE FOR STEADY STATE RECORD')
 8027 FORMAT ('0DATA REC ',I11,': DURATION MAY NOT BE MODELLED FOR STEADY STATE INFUSION')
 8028 FORMAT ('0DATA REC ',I11,': MDV DATA ITEM MUST BE 1 FOR LAST RECORD OF OTHER EVENT')
 8029 FORMAT ('0DATA REC ',I11,': OTHER EVENT RECORD MAY NOT SPECIFY DOSING INFORMATION')
 8030 FORMAT ('0DATA REC ',I11,': SPECIFIED COMPARTMENT MAY NOT BE TURNED ON OR OFF')
 8031 FORMAT ('0DATA REC ',I11,': MAXIMUM NUMBER OF ERRORS EXCEEDED - NO MORE DATA RECORDS'/  &
                              ' WILL BE CHECKED')
 8032 FORMAT ('0DATA REC ',I11,': NUMBER OF CONTINUATION RECORDS FOR THE EVENT RECORD EXCEEDS', I3)
 8034 FORMAT ('0DATA REC ',I11,': PK PARAMETER FOR DURATION IS NOT SUPPLIED BY PK FOR'/' COMPT. NO. ',I3)
 8037 FORMAT ('0DATA REC ',I11,': COMPARTMENT ASSOCIATED WITH THE PREDICTION IS OFF: CMT=',I5)
 8038 FORMAT ('0DATA REC ',I11,': NEGATIVE TIME DATA ITEM IS INAPPROPRIATE')
 8039 FORMAT ('0DATA REC ',I11,': SS DOSE MAY NOT BE USED WITH NULL SSS ROUTINE')
 8040 FORMAT ('0DATA REC ',I11,': DEFAULT OUTPUT COMPARTMENT MUST BE OFF WHEN SS DOSE IS GIVEN')
 8058 FORMAT ('0DATA REC ',I11,': MDV DATA ITEM MUST BE 1 FOR LAST RECORD OF RESET EVENT')
 8059 FORMAT ('0DATA REC ',I11,': RESET EVENT RECORD MAY NOT SPECIFY DOSING INFORMATION')
 8060 FORMAT ('0DATA REC ',I11,': ADDITIONAL DOSES DATA ITEM MUST BE A NON-NEGATIVE INTEGER')
 8061 FORMAT ('0DATA REC ',I11,': INTERDOSE INTERVAL DATA ITEM IS ZERO,'/                     &
 ...
 8118 FORMAT ('0DATA REC ',I11,': INTERDOSE INTERVAL DATA ITEM IS NON-ZERO,'/                 &
                              ' ADDITIONAL DOSES DATA ITEM IS ZERO')
 8218 FORMAT ('0DATA REC ',I11,': INTERDOSE INTERVAL DATA ITEM IS NON-ZERO,'/                 &
